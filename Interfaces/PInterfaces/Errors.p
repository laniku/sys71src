{
Created: Saturday, July 27, 1991 at 8:38 PM
 Errors.p
 Pascal Interface to the Macintosh Libraries
  ...
  All rights reserved
	...
	Change History (most recent first):
	   ...
	   <SM6>	  8/3/93	BH		Added manual-eject errors: dsRemoveDisk, dsDirtyDisk.
		<25>	11/13/92	JDR		Added component manager errors.
		<24>	 9/03/92	JDR		Changed noMaskFound into noMaskFoundErr.
		<23>	  9/3/92	DCL		Moved IconUtils error noMaskFound from private to public
									interfaces.
		<22>	 6/25/92	hsK		Use the BTree manager error code names instead of the Dictionary
									manager error code names.
		<21>	 5/27/92	hsK		#1030902,<YK>: Add the Dictionary manager error numbers.
		<20>	 9/17/91	MH		Remove the last four lines of Using Include stuff (added <19>)
									Needed only if the present file includes others.
		<19>	 7/30/91	JL		Updated Copyright. Added ErrorsIncludes conditional settings.
		<18>	 6/14/91	JL		Checked in official MPW 3.2ƒ version. Added error constants :
									afpContainsSharedErr - > afpInsideTrashErr (-5033 - -5044) for
									the File System. Updated created date to March 14th. (2nd line
									this file.)
		<17>	 1/28/91	LN		Checked in Database Generated File from DSG. Temporaily lost
									some comments.
		<16>	 1/16/91	JDR		(dba) Moved the Process Mgr error numbers into the Errors
									interface file.
		<15>	12/14/90	dba		<JDR> add fontNotOutlineErr
		<14>	 12/5/90	dba		<dnf> add new system error for setting a bad trap address;
		...
		<13>	11/15/90	JDR		<KIP> Removed the Sound Input Manager errors and put them into
									the Errors.p
		<12>	11/14/90	JDR		<KIP> Moved all the Sound Manager errors into the Sound.p.
		<11>	11/13/90	EMT		<stb> Add noUserNameErr
		<10>	 11/8/90	EMT		Move PPCError codes to in this file
		 <9>	10/30/90	KST		Adding new HFS error codes.
		 <8>	 9/18/90	DDG		Added error codes for picture utilities.
		 <7>	 8/21/90	KON		Add error codes -145, -147, -149.
		 <4>	  1/4/90	ngk		Added Edition Manager error codes
		<2>	 	  1/3/90	pp		Add userCanceledErr for passing a status of user cancelling out
									of an operation.
		<1.2>	 9/29/89	EKN		Changed file ID errors and the range from -130 to -140 TO -1300 to -1500.
		<1.1>	 7/31/89 	EKN		Added FileIDs errors -130 to -134
}
...
noDataArea = -1104;
reqAborted = -1105;
invalidComponentID = -3000;					{errors from Component Manager & components}
...
dsRemoveDisk = 20003;						{request user to remove disk from manual eject drive  <SM6> <BH 03Aug93>}
