...
Created: Saturday, July 27, 1991 at 3:09 PM
 Errors.h
 C Interface to the Macintosh Libraries
  ...
  All rights reserved
	...
	Change History (most recent first):
	  ...
	  <SM10>	  2/6/94	pdw		Removed trailing comma to avoid C warning.
	   <SM9>	 1/13/94	chp		Moved some serial driver bit definitions to Serial.h since they
									aren’t system errors or result codes.
	   <SM8>	  8/3/93	BH		Added manual-eject errors: dsRemoveDisk, dsDirtyDisk.
		<32>	 6/15/93	JDR		All errors belong here. Moved Speech Manager errors back to
									where they belong.
		<31>	  6/3/93	DTY		Synch up with NuReality.
		<30>	  6/2/93	DTY		Include ConditionalMacros.h.
		<29>	  6/2/93	MC		Moved Speech Mgr errors back to Speech.h!
		<28>	 3/10/93	JDR		Added speech manager errors.
		<27>	  3/7/93	DTY		Cast badComponentInstance and badComponentSelector to longs.
		<26>	  3/3/93	JDR		Added QuickTime errors and did some housekeeping.
		<25>	 1/18/93	ash		Added translation manager errors
		<24>	12/12/92	DRF		Fix a trailing comma inside an enum so we can compile under MPW
									C 3.2.4
		<23>	11/13/92	JDR		Added component manager errors.
		<22>	 9/03/92	JDR		Changed noMaskFound into noMaskFoundErr.
		<21>	  9/3/92	DCL		Moved IconUtils error noMaskFound from private to public
									interfaces.
		<20>	 6/25/92	hsK		Use the BTree manager error code names instead of the Dictionary
									manager error code names.
		<19>	 5/27/92	hsK		#1030902,<YK>: Add the Dictionary manager error numbers.
		<18>	 3/24/92	KSM		SysError inline just pulled the parameter off the stack into D0
									and called _SysError.  Using the pragma generates better code.
		<17>	 7/30/91	JL		Updated Copyright.
		<16>	 6/14/91	JL		Checked in official MPW 3.2ƒ version. Added error constants :
									afpContainsSharedErr - > afpInsideTrashErr (-5033 - -5044) for
									the File System. Adjusted enum grouping. Updated created date to
									March 14th. (3rd line this file.)
		<15>	 1/28/91	LN		Checked in Database generate file from DSG. Grouping enums into
									groups of 20 for C++ support.
		<14>	 1/16/91	JDR		(dba) Moved the Process Mgr errors to here.
		<13>	12/14/90	dba		<JDR> add fontNotOutlineErr
		<12>	 12/5/90	dba		<dnf> add new system error for setting a bad trap address;
		...
		<11>	11/15/90	JDR		<KIP> Removed the Sound Input Manager errors and put them into
									the Errors.h
		<10>	11/14/90	JDR		<KIP> Moved all the Sound Manager errors into the Sound.h.
		 <9>	11/13/90	EMT		<stb> Add noUserNameErr
		 <8>	 11/8/90	EMT		<stb>Move PPCError codes to in this file
		 <7>	 9/18/90	DDG		Added error codes for picture utilities.
		 <6>	 8/21/90	KON		Add error codes -145, -147, -149.
		 <3>	  1/7/90	ngk		Add EditionMgr error codes
		 <2>	  1/3/90	PP		Add userCanceledErr for passing a status of user cancelling out
									of an operation.
		<1.4>	  9/29/89  	EKN 	Changed file ID errors and the range from -130 to -140 TO -1300 to -1500.
		<1.3>	  9/05/89	dnf 	Added CatChangedErr
		<1.2>	  7/31/89 	EKN 	Added FileIDs errors -130 to -134
...
enum {
 ...
 dataNoDataRef = -2047,
 noMovieFound = -2048,							/*private errors*/
...
enum {
 ...
 dsRemoveDisk = 20003,							/*request user to remove disk from manual eject drive  <SM8> <BH 03Aug93>*/
 dsDirtyDisk = 20004,							/*request user to return a manually-ejected dirty disk <SM8> <BH 03Aug93>*/
