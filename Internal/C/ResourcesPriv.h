/*
	...
	Change History (most recent first):
		...
		<15>	 5/27/93	JSM		Make routine prototypes C++ friendly.
		<14>	 2/24/93	JDR		Need to have at least one element in an array for proper C
									syntax, so I changed the ResourceMap structure from zero length
									arrays to one element.
		<13>	 1/20/93	DTY		Renamed from ResourceMgrPriv.h.
		<12>	  1/4/93	DTY		Add mask and bit constants for mapIsOverridden.
...
#ifndef __RESOURCESPRIV__
#define __RESOURCESPRIV__
...
struct ResourceMap {
	...
	ReferenceEntry		referenceList[1];			// Reference entries for all resources in this file
	char				nameList[1];				// String of resource names
};
...
#define	kMapIsOverriddenMask			32
